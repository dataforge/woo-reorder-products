...
class Woo_Reorder_Products_Updater {

	const GITHUB_REPO = 'dataforge/woo-reorder-products';
	const SLUG        = 'woo-reorder-products';
	const CACHE_KEY   = 'woo_reorder_products_github_release';
	...
	public static function init() {
		...
		add_action( 'admin_post_woo_reorder_products_check_updates', array( __CLASS__, 'handle_check_updates' ) );
		add_filter( 'plugin_action_links_' . plugin_basename( WOO_REORDER_PRODUCTS_FILE ), array( __CLASS__, 'action_links' ) );
	}

	public static function check_update( $update, $plugin_data, $plugin_file, $locales ) {
		if ( plugin_basename( WOO_REORDER_PRODUCTS_FILE ) !== $plugin_file ) {
			return $update;
		}
	...
	public static function fix_directory( $result, $options ) {
		...
		if ( ! isset( $options['plugin'] ) || plugin_basename( WOO_REORDER_PRODUCTS_FILE ) !== $options['plugin'] ) {
			return $result;
		}
	...
	public static function plugin_info( $result, $action, $args ) {
		...
		$info               = new stdClass();
		$info->name         = 'Woo Reorder Products';
		$info->slug         = self::SLUG;
	...
	public static function handle_check_updates() {
		...
		check_admin_referer( 'woo_reorder_products_check_updates' );
		...
		if ( $release ) {
			$remote_version = (string) preg_replace( '/^v/', '', (string) $release->tag_name );
			if ( version_compare( WOO_REORDER_PRODUCTS_VERSION, $remote_version, '<' ) ) {
				$status = 'update_available';
			}
		}

		wp_safe_redirect( add_query_arg( array( 'update_check' => $status ), admin_url( 'admin.php?page=woo-reorder-products' ) ) );
		exit;
	}
	...
	public static function is_update_available() {
		...
		return version_compare( WOO_REORDER_PRODUCTS_VERSION, $remote_version, '<' );
	}

	public static function action_links( $links ) {
		$url  = wp_nonce_url(
			admin_url( 'admin-post.php?action=woo_reorder_products_check_updates' ),
			'woo_reorder_products_check_updates'
		);
